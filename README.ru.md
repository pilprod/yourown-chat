...
3. **Создать Cloudflare API-токен** (zone-scoped, единственный статический
   секрет) и положить в HCP variable set, привязанный к стеку cloudflare.
...
6. **Выкатить workloads** из [`helm/`](helm/README.md): ingress-nginx +
   оператор Mattermost, заменить маркеры `REPLACE-ME-*`, применить манифесты.
