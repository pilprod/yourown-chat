...
deployment "eu" {
  inputs = {
    ...
    # Closed, production-ineligible vendor baseline. Product-owned values are
    # copied byte-for-byte and checksum-verified by the public generic adapter.
    ...
    # Route stays closed through database/chart installation and opens only in
    # a follow-up catalog revision after internal readiness has been verified.
    private_http_routes = {
      kagent = {
        enabled   = false
        namespace = "kagent-system"
...
publish_output "vendor_chart_bundles" {
  ...
  value       = deployment.eu.vendor_chart_bundles
}

publish_output "additional_database_users" {
  description = "Additional logical database/user requests for the shared Cloud SQL instance."
  value       = deployment.eu.additional_database_users
}

publish_output "private_http_routes" {
  description = "Typed private HTTP routes consumed by the Cloudflare Access/Tunnel Stack."
  value       = deployment.eu.private_http_routes
}
