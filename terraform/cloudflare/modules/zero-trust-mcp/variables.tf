variable "account_id" {
  ...
  validation {
    ...
    error_message = "account_id must not be empty when Zero Trust MCP is enabled."
  }
}
...
variable "upstreams" {
  type        = map(string)
  description = "Hostname label => in-cluster service URL (e.g. mcp-terraform => http://mcp-terraform.mattermost.svc.cluster.local:8080). One tunnel ingress rule, DNS record and Access app per entry."

  validation {
    condition     = length(var.upstreams) > 0
    error_message = "Provide at least one upstream when Zero Trust MCP is enabled."
  }
}
