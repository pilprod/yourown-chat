...
# pool. Resources are named by role (Workload Identity SAs mattermost /
# mattermost-dev / matterbridge) or regionally (europe-west3-*), never by
# environment (environment drives labels, not names); the dev tenant's GCP
# objects (its Workload Identity SA `mattermost-dev`, its in-cluster Postgres
# password secret) are created by this one deployment.
#
