...
every IP destination remains an exact `/32`. DNS egress retains the exact
Service ClusterIP and selects both `kube-dns` and `node-local-dns` pods in
`kube-system`; GKE Dataplane V2 does not use `ipBlock` rules for Pod IP
destinations. The application release stays absent until every declared
database connection secret ID has been published by the platform Stack. The CRD
