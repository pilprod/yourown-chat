locals {
  # Regional singletons named after the region; global objects (VPC, firewall,
  # PSA) by bare role. Secondary ranges keep the ${region}-pods/-services names
  # because renaming a subnet field replaces the subnet (and the live cluster).
  network_name           = "vpc"
...
# Private Service Access: reserved range + peering for private-IP reach to
...
# Reserved regional external IP for the public ingress LB (prod only). Stable
# across LB re-creations so the Cloudflare DNS record is never orphaned.
