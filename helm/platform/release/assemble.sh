...
#   <out>/verify/<wrapper>.yaml    Cloud Deploy verification Job manifest per wrapper
...
for w in m["wrappers"]:
    ...
    seen_checks = set()
    for check in (w.get("verify") or {}).get("http", []):
        if check["name"] in seen_checks:
            sys.exit(f"wrapper {w['name']} declares verification check {check['name']} twice")
        seen_checks.add(check["name"])
        print(f"check|{w['name']}|{check['name']}|{check['url']}|{check.get('retries', 30)}|{check.get('delaySeconds', 5)}")
PY
...
PY
  ...
  # A wrapper that deploys a network service or stateful workload must declare
  # functional verification; workers and jobs are verified by rollout status
  # and completion.
  wrapper_checks="$(printf '%s\n' "${plan}" | awk -F'|' -v w="${name}" '$1 == "check" && $2 == w { print }')"
  if grep -Eq '"name": *"platform-(service|stateful)"' "${evidence}/${name}-chart.json" && [ -z "${wrapper_checks}" ]; then
    die "wrapper ${name}: pins platform-service or platform-stateful and must declare verify.http checks in the release manifest"
  fi
  ...
  for entry in "${profiles[@]}"; do
    ...
    if [ -n "${wrapper_checks}" ]; then
      mkdir -p "${out}/verify"
      cat > "${out}/verify/${name}.yaml" <<JOB
# Generated by helm/platform/release/assemble.sh. Cloud Deploy replaces the
# verify container with the checks declared in the release manifest.
apiVersion: batch/v1
kind: Job
metadata:
  name: platform-verify-${name}
  namespace: ${namespace}
  labels:
    app: verify
    app.kubernetes.io/name: platform-verify
...
JOB
      {
        printf '      - name: %s-verify\n' "${name}"
        printf '        container:\n'
        printf '          name: verify\n'
        printf '          image: curlimages/curl:8.10.1\n'
        printf '          command: ["/bin/sh", "-ceu"]\n'
        printf '          args:\n'
        printf '            - |\n'
        while IFS='|' read -r ckind cwrapper cname curl cretries cdelay; do
          [ "${ckind}" = "check" ] || continue
          printf '              echo "[platform-verify] %s"\n' "${cname}"
          printf '              curl -sSf --connect-timeout 3 --max-time 10 --retry %s --retry-all-errors --retry-delay %s "%s"\n' "${cretries}" "${cdelay}" "${curl}"
          printf '              echo\n'
        done <<<"${wrapper_checks}"
        printf '        executionMode:\n'
        printf '          kubernetesCluster:\n'
        printf '            jobManifestPath: verify/%s.yaml\n' "${name}"
      } >> "${skaffold_releases_dir}/${profile}.verify"
      printf 'check\t%s\t%s\n' "${name}" "$(printf '%s\n' "${wrapper_checks}" | awk -F'|' '{ printf "%s=%s;", $3, $4 }')" >> "${facts}"
    fi
...
SKAFFOLD
  for entry in "${profiles[@]}"; do
    ...
    if [ -f "${skaffold_releases_dir}/${profile}.verify" ]; then
      printf '    verify:\n'
...
for line in open(facts_path):
    ...
    if kind == "chart":
        ...
        w["verification"] = [{"name": c.split("=", 1)[0], "url": c.split("=", 1)[1]} for c in parts[2].split(";") if c]
